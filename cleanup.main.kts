...
// val latestVersionRegex =
//     "<latest>((?:[a-z,0-9]{7})|(?:\\d{1,3}\\.\\d{1,3}\\.\\d{1,4}))-.*<\\/latest>".toRegex(RegexOption.MULTILINE)
val versionRegex =
...
val features = searchFolders("features")
...
printFooter()
printHeader("Feature versions")
val featureVersions = searchLatestVersions(features)
printFooter()
...
printHeader("Clean Feature Versions")
cleanVersions(features)
printFooter()
...
fun cleanVersions(folders: Sequence<File>) {
    folders.forEach { folder ->
        ...
        val latestVersion = releaseVersions[name] ?: snapshotVersions[name] ?: featureVersions[name]
